	...
	router.get('/callback_github', function (req, res) {
		...
		function saveGithubLogin(error, response, body) {
			if (error) {
				...
				if (api_resp
						&& (req.session.pseudo = api_resp.login)
						&& (req.session.oauth = 'github'))
				{
					users.findOne({"pseudo" : api_resp.login, "oauth": "github"}, function(err, user) {
						if (err) throw err;
						if (!user) {
							users.insertAsync({
								'first_name'  : api_resp.name,
								'last_name' 	: api_resp.name,
								'email' 			: api_resp.email,
								'pseudo'			: api_resp.login,
								'oauth'				: 'github'
							});
						}
					});
		...
		function save42Login(error, response, body) {
			if (error) {
				...
				if (api_resp
					...
					users.findOne({"pseudo" : api_resp.login, "oauth": "42"}, function(err, user) {
						if (err) throw err;
						if (!user) {
							users.insertAsync({
	...
	router.get('/callback_google', function (req, res) {
		...
		function saveGoogleLogin(error, response, body) {
			...
			if (error) {
				...
				var api_resp = JSON.parse(body);
				if (api_resp
							&& (req.session.pseudo = api_resp.name)
							&& (req.session.oauth = 'google')) {

					users.findOne({"pseudo" : api_resp.name, "oauth": "google"}, function(err, user) {
						if (err) throw err;
						if (!user) {
							users.insertAsync({
								'first_name'  : api_resp.given_name,
								'last_name' 	: api_resp.family_name,
								'email' 			: api_resp.email,
								'pseudo'			: api_resp.name,
								'oauth'				: "google"
							});
						}
					});
		...
		function saveRedditLogin(error, response, body) {
			if (error) {
				...
				if (api_resp
					...
					users.findOne({"pseudo" : api_resp.name, "oauth": "reddit"}, function(err, user) {
						if (err) throw err;
						if (!user) {
							users.insertAsync({
								'first_name'  : api_resp.name,
